...
const PieChart: React.FC<ChartProp> = ({ chartList }) => {
	...
	const data = {
		labels: chartList?.map((item) => tags[item.tag] || item.tag) || emptyChartData.labels,
		datasets: [
			{
				data: chartList && chartList.length > 0 ? chartList.map((item) => item.count) : emptyChartData.datasets[0].data,
				backgroundColor:
