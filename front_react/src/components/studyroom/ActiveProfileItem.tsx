...
const ActiveProfileItem = ({ person, rank }: { person: IRanking; rank: number }) => {
	...
	return (
		<div>
			<div
				className={`${
					isSolving
						? 'from-primary/50 via-secondary/50 to-blueishPurple/50'
						: 'from-primary/50 via-secondary/50 to-blueishPurple/50'
				} p-[2px] bg-gradient-to-br w-fit rounded-2xl mr-2 shadow-foggyBlue`}
			>
				<div
					className={`${
						isSolving ? 'bg-opacity-0' : null
					} bg-white w-40 h-52 rounded-2xl flex flex-col items-center p-4`}
				>
					{/* gradient border */}
					<div className='h-[60%] flex items-center justify-center'>
						<img src={person.imageUrl} alt='프사' width={72} height={72} className='border rounded-full' />
					</div>
