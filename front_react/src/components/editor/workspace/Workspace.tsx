  ...
  useEffect(() => {
    ...
    const handleSelectionChange = () => {
      ...
      if (selection && selection.rangeCount > 0) {
        ...
        const isTableCell =
          selectedNode.closest('td') || selectedNode.closest('th')
        const isImage = selectedNode.closest('img')

        if (
          !range.collapsed &&
          workspaceRef.current?.contains(selection.anchorNode) &&
          !isTableCell &&
          !isImage
        ) {
  ...
  return (
    <div className="editor w-full h-full" ref={workspaceRef}>
      <div className="relative top-bar-container">
        <MdMenu
          className="w-7 h-7 absolute right-4 top-0 m-2 bg-transparent text-black hover:bg-navy/20 p-1 rounded z-10"
      ...
      >
        <div
          className="bg-white z-10 rounded-md"
          style={{
            position: 'absolute',
