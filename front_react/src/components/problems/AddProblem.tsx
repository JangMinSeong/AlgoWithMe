	...
	return (
		<div onClick={handleModal} className='w-full '>
			<div className='flex w-full justify-between bg-background grow h-[72px] items-center px-4 py-4 rounded-lg border border-blueishPurple border-opacity-30 shadow-foggyBlue mb-2 hover:bg-purple-200 hover:border-opacity-0 transition-colors'>
				...
				<div className=' font-bold flex justify-center mr-2 w-36'>문제 추가하기</div>
				<div className='flex'>
					<img src='/swea.png' alt='swea logo' width={20} height={20} className='rounded-full mr-2' />
					<img src='/bojlogo.png' alt='boj logo' width={20} height={20} className='rounded-full mr-2' />
					<img src='/programmers.png' alt='programmers logo' width={20} height={20} className='rounded-full' />
				</div>
			</div>
			{showModal && <AddProblemModal clickModal={clickModal} groupId={groupId} type='addCandidates' />}
		</div>
	)
}
