...
const API_URL = import.meta.env.MODE === 'development' ? import.meta.env.VITE_API_DEV_URL : import.meta.env.VITE_API_URL
...
const AddProblemModal = ({
	...
	const existingCandidateProblems = useSelector((state: RootState) => state.study.candidateProblems)
	const handleAddCandidate = async () => {
		...
		if (duplicated()) {
			...
			const addCandidate = await fetch(`${API_URL}/study/problem`, {
				...
				body: JSON.stringify({ teamId: groupId, problemId: chosenProblem.id }),
				credentials: 'include',
			})
	...
	return (
		<div onClick={(e) => clickModal(e)} className='z-10 top-0 left-0 fixed bg-black/30 w-lvw h-lvh'>
			<div
			...
			>
				...
				<div className='absolute bottom-2 '>
					<Button onClick={(e) => clickModal(e)} variant='secondary' className='mr-2'>
						취소하기
					</Button>
					{type === 'addCandidates' ? (
