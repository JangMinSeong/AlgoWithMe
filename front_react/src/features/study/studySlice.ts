...
const studyState = createSlice({
	...
	reducers: {
		...
		addCandidateProblems: (state: IStudyState, action: PayloadAction<IProblemInfo>) => {
			state.candidateProblems = [...state.candidateProblems, action.payload]
		},
	},
})

export const { viewStudyInfo, editImage, editName, addCandidateProblems } = studyState.actions
