...
public class TeamController {
  ...
  @ApiResponses(value = {
      @ApiResponse(responseCode = "200", description = "생성 성공", content = {
          @Content(schema = @Schema(implementation = TeamInfoResponse.class))}),
  ...
  @ApiResponses(value = {
      @ApiResponse(responseCode = "200", description = "추가 성공", content = {
          @Content(schema = @Schema(implementation = AddProblemResponse.class))}),
      @ApiResponse(responseCode = "400", description = "추가 실패"),
  })
  public ResponseEntity<AddProblemResponse> addCandidateProblem(
      @RequestBody AddProblemRequest request, @AuthenticationPrincipal User user) {
  ...
  public ResponseEntity<InviteUrlResponse> createInviteUrl(@PathVariable("teamId") Long teamId,
                                                           @AuthenticationPrincipal User user) {
    return ResponseEntity.ok(teamService.createInviteUrl(teamId, user));
  }

  @GetMapping("/member")
  public ResponseEntity<String> addMember(@RequestParam("encrypted") String encrypted,
                                          @AuthenticationPrincipal User user) {
  ...
  public ResponseEntity<TeamInfoDetailResponse> getTeamInfoDetail(
      @AuthenticationPrincipal User user,
